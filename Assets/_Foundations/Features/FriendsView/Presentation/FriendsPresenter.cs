...
namespace FriendsView.Presentation
{
    ...
    public class FriendsPresenter
    {
        ...
        {
            webClient.GetPersonalUserData()
                .Do(uS =>
                {
                    userData = uS;
                    webClient.SetPersonalData(uS);
                })
                .Do(friendsNotificationsServices.OpenServices)
                .DoOnError(Debug.Log)
            ...
            this.view.OnStart
                .Subscribe(SubscribeAvatarTaps).AddTo(sectionDisposables);
        ...
        void SubscribeAvatarTaps()
        {
            // playerWorldData.OnTapAvatar
            //     .Do(r =>
            //     {
            //         if (view.Card.gameObject.activeInHierarchy) return;
            //
            //         view.Card.SetFriendCardLoading(RemotePlayersPool.Current.GetOnUsedPlayer(r.remotePlayerId)
            //             .username);
            //         view.Card.gameObject.SetActive(true);
            //         view.Card.snapshotAvatar.CreateSnaphot(null, r.customization.AvatarCustomizationData);
            //     })
            //     .SelectMany(r => webClient.GetUser(r.remotePlayerId))
            //     .Subscribe(PresentAtWorldCard)
            //     .AddTo(disposables);
        }
        ...
        void NotifyNewFriend(string newUser, string firebaseUid, List<FriendData> friendsList)
        {
            ...
            var onUsedPlayer = RemotePlayersPool.Current.GetOnUsedPlayer(firebaseUid);

            if (onUsedPlayer == null) return;

            onUsedPlayer.Notifications.PlayNotification();
        }
        ...
        {
            view.SetPersonalCard(userData);

            webClient.GetPersonalUserData()
                .Do(uS => { view.SetPersonalCard(uS); })
                .DoOnError(Debug.Log)
        ...
        async void VisitFriend(FriendData friendData)
        {
            ...
            if (updatedData.inPublicArea)
            {
                if (string.IsNullOrEmpty(updatedData.chatRoomName) || string.IsNullOrEmpty(updatedData.chatRoomId))
                    return;

               // await roomLoader.ConnectNewRoomAsync(updatedData.chatRoomName, updatedData.chatRoomId);
            }
            else
            {
